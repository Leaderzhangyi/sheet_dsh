...
function App() {
  ...
  const currentLocation = (): NavigationState => ({
    activeWorkspace,
    activeNav,
    query,
    searchFilter,
    ...
    <div className="table-detail" data-testid="table-detail">
      ...
      <div className="fields-toolbar">
        ...
        <div className="field-toolbar-right">
          ...
          <button className="icon-button" title="字段设置">
            <Settings2 size={16} />
          </button>
        </div>
      </div>
...
function CodesView({
  ...
  dataset: DictionaryDataset;
  selectedCode: DictionaryDataset["codeItems"][number] | null;
  onSelect: (code: DictionaryDataset["codeItems"][number]) => void;
  onOpenField: (field: FieldRecord) => void;
}) {
  const [filter, setFilter] = useState("");
  const [visible, setVisible] = useState(80);
...
function originLabel(origin: DatasetOrigin) {
  return origin === "cache"
    ? "本地索引"
    : origin === "uploaded"
      ? "本地导入"
